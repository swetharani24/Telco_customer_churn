...
def plot_tenure_vs_churn(df):
    ...
    tenure_churn_q = pd.crosstab(
        df['Tenure_Quarter'],
        df['Churn']
    )

    print(tenure_churn_q)


    tenure_churn_q.plot(kind='bar')
...
def plot_simtype_vs_gender(df):
    ...
    # Count of male/female per SIM operator
    sim_gender_counts = pd.crosstab(df['SIM_Operator'], df['gender'])
    print(sim_gender_counts)
    sim_gender_counts.plot(kind='bar')
    plt.xlabel("SIM Operator")
    plt.ylabel("Number of Customers")
    plt.title("SIM Operator Distribution by Gender (Grouped Bar Chart)")
    plt.xticks(rotation=0)
